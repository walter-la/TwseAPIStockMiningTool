    ...
    public partial class Form1 : Form
    {
        ...
        public bool IsTradeTime
        {
            get
            {
                ...
                return new TimeSpan(8, 59, 0) <= nowTime && nowTime <= new TimeSpan(14, 31, 0) && DayOfWeek.Sunday < dayOfWeek && dayOfWeek < DayOfWeek.Saturday;
            }
        }
        ...
        void time_Tick(object sender, EventArgs e)
        {
            if (IsTradeTime)
            {
                RequestTwse(CombineUrl, _allStockInfoFormatString);
            }
        }


        private static void RequestTwse(string CombineUrl, string _allStockInfoFormatString)
        {

            var req = HttpWebRequest.Create(CombineUrl) as HttpWebRequest;
            ...
            {
                writer.Write(string.Format(_allStockInfoFormatString, DateTime.Now.AddDays(-1).ToString("yyyyMMdd")));
            }
            DateTime RequestTime = DateTime.Now;
            double TimeRange = 0.0;
            RecordMessage("系統時間：" + DateTime.Now.ToString("yyyyMMdd HH:mm:sss"));

            try
            {
                using (var res = req.GetResponse() as HttpWebResponse)
                {
                    TimeRange = new TimeSpan(res.LastModified.Ticks - RequestTime.Ticks).TotalMilliseconds;
                    RecordMessage(res.LastModified.ToString("yyyyMMdd HH:mm:sss"));
                    RecordMessage("差異時間（毫秒）：" + TimeRange);
                    var jsonContent = new StreamReader(res.GetResponseStream()).ReadToEnd();
                    //RecordMessage(jsonContent);
                }
            }
            catch (Exception err)
            {
                RecordMessage(err.Message + Environment.NewLine + err.StackTrace);
                TimeRange = 0;
            }
            TimeData.Add((long)(DateTime.Now.ToUniversalTime() - UnixTime).TotalSeconds, TimeRange);
        }
        ...
        private void BtnIP_Click(object sender, EventArgs e)
        {
            if (!time.Enabled)
            {
                CombineUrl = string.Format(StockInfoUrl, Ip);
                time.Start();
            }
        }
        ...
        private void BtnDomain_Click(object sender, EventArgs e)
        {
            if (!time.Enabled)
            {
                CombineUrl = string.Format(StockInfoUrl, Domain);
                time.Start();
            }
        }
        ...
        {
            using (StreamWriter sw = new StreamWriter(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, DateTime.Now.ToString("yyyy_MM_dd HH_mm_ss") + cboTestList.Text + "log.txt"), true))
            {
        ...
        {
            if (time.Enabled || string.IsNullOrEmpty(cboTestList.Text))
                return;

            CombineUrl = string.Format(StockInfoUrl, cboTestList.Text);
            btnTest.Enabled = cboTestList.Enabled = false;
            time.Enabled = true;
            time.Start();
        }
    }
